...
4. **Convert** — click **Convert to Live Photo**. Video2LivePhoto will:
...
A Live Photo is just a paired HEIC + MOV with matching metadata. Video2LivePhoto handles the entire pipeline correctly — frame extraction, metadata embedding, and Photos import — in one click.
...
- **"Photos library access was denied"** → System Settings → Privacy & Security → Photos → enable Video2LivePhoto.
...
- 🐦 Share it on X / 小红书 / Threads — tag with `#Video2LivePhoto`.
